 ...
 * 
 * out.println(); ()안의 내용을 출력 한 후 줄바꿈
 * out.print(); ()안의 내용을 출력 한 후 줄바꿈하지않음. 
...
public class PrintExample {
	...
	public static void main(String[] args) {
		...
		/*
		 	...
		 	\\ 역슬러시(\)를 출력
		 */
		String str = "나는 오늘 \"오만과편견\" 이라는 책을 읽었습니다.";
		System.out.println(str);
		
		
		/*
		 * 	약속된 형식을 지정해놓고 사용하는 출력
		 	printf - 줄바꿈이 이루어지지 않음
		 	출력하려는 지시자(값)의 수만큼 필요한 값을 알려줘야함.
		...
		%[-][0][n][.m][,m]
	 	...
	 	%[n] : 출력할 전체 자리수를 n만큼 지정(오른쪽 정렬)
	 	%5d, 1000 == [ ][1][0][0][0]
	 	%[-] : 전체 자리수가 지정된 경우 왼쪽으로 정렬하고 빈칸에 공백 출력
	 	%-5d, 1000 == [1][0][0][0][ ] 
	 	%[0] : 전체자리수가 지정되어 있을 때 빈자리를 0으로 표현
	 	%05d, 1000 == [0][1][0][0][0]
